...
mapping = {0: '0 - Detective', 1: '1', 2: '2 - Police', 3: '3 - Drug traces',
           ...
           8: '8 - Drug traces', 9: '9 - Drug traces', 10: '10 - Drug racket location'}
H = nx.relabel_nodes(G, mapping)
pos = nx.spring_layout(H)
nx.draw_networkx_nodes(H, pos, node_size=[200]*len(H.nodes()))
nx.draw_networkx_edges(H, pos)
nx.draw_networkx_labels(H, pos)
pl.show()

# Environment-aware Q-learning
Q = np.matrix(np.zeros([MATRIX_SIZE, MATRIX_SIZE]))
...
def available_actions_with_env_help(state, env_matrix_snap):
    current_state_row = M[state, ]
    av_action = np.where(current_state_row >= 0)[1]
...
# Train with environmental awareness
scores = []
for i in range(1000):
    current_state = np.random.randint(0, int(Q.shape[0]))
    available_action = available_actions(current_state)
...
for i in range(1000):
    current_state = np.random.randint(0, int(Q.shape[0]))
    available_action = available_actions_with_env_help(current_state, env_drugs - env_police)
    action = sample_next_action(available_action)
    score = update(current_state, action, gamma)
    scores.append(score)
